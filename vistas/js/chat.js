...
function multiplesArchivos(archivo){
  ...
  $(datosArchivo).on("load", function(event){
    ...
    if(archivo["type"] == "image/jpeg" || archivo["type"] == "image/png"){
      ...
      <li>
            <span class="mailbox-attachment-icon has-img"><img src="`+rutaArchivo+`" alt="Attachment"></span>

            <div class="mailbox-attachment-info">
              <a href="#" class="mailbox-attachment-name"><i class="fa fa-camera"></i>`+archivo['name']+`</a>
                  <span class="mailbox-attachment-size">
                   <span>`+archivo['size']+` Bytes</span>
                    <button type="button" class="btn btn-danger btn-xs pull-right quitarAdjunto" temporal><i class="fa fa-times"></i></button>
                  </span>
            </div>
          </li>
    ...
    method: "POST",
      ...
      success:function(respuesta){
        ...
        if(respuesta == "ok"){
          ...
          swal({
              type:"success",
                title: "¡MENSAJE ENVIADO CORRECTAMENTE A LA PAPELERA!",
                text: "¡Puede recuperar el mensaje si lo desea en la sección de papelera!",
        ...
        if(respuesta == "recuperado"){
          ...
          swal({
              type:"success",
                ...
                text: "¡Revisa en las otras bandejas el mensaje recuperado!",
                showConfirmButton: true,
              confirmButtonText: "Cerrar"
...
$(".tablaMsj").on("draw.dt", function(){
  ...
  /*=============================================
  ENVIAR MENSAJES DE FORMA MASIVA A LA PAPELERA
  =============================================*/
  ...
  for(var i = 0; i < ticketCheckbox.length; i++){
    ...
    /*=============================================
      ...
      $(ticketCheckbox[i]).on("ifUnchecked", function(event){
        ...
        for(var j = 0; j < quitarTicketsPapelera.length; j++){

          if(quitarTicketsPapelera[j] == $(this).attr("idTicket")){

            quitarTicketsPapelera.splice(j, 1);

            idTickets.splice(j, 1);
